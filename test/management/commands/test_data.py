...
def generate_random_data_with_names():
    ...
    for name in names_list:
        for session in ['Game', 'Training']:
            for _ in range(3):
                date = (datetime(2023, 1, 1) + timedelta(days=np.random.randint(-30, 365))).strftime('%d-%m-%Y')
                ...
                step_counts = np.random.randint(8500, 10500)
                ...
                calorie_expenditure = np.random.randint(500, 800)
...
# Save to a CSV file
output_file_path_with_names = 'C:/Users/14391/Desktop/SD/random_athlete_data_with_names.csv'
